...
class Study:
    ...
    def process_effectiveness(self):
        ...
        prompt = prompt.format(abstract=self.data["abstract"],json=json_object,target_drug=self.drug_name)
        prompt_dict = {}
        prompt_dict["prompt"] = prompt
        result = gpt_request_from_relative_chunk(prompt_dict)
        try:
    ...
    def process_pediatrics_incontent(self):
        ...
        relative_chunk = find_relative_chunk( os.path.join("/LLM-Medical-Agent/data/json_100",self.drug_name,self.abstract_url+".json"),question)
        content = []
        all_token = 0
    ...
    def process_population_effectiveness(self):
        ...
        prompt = prompt.format(target_drug = self.drug_name,abstract=self.data["abstract"],json=json_object)
        prompt_dict = {}
        prompt_dict["prompt"] = prompt
        result = gpt_request_from_relative_chunk(prompt_dict)
        try:
